...
import com.lee.function.Msg;
import com.lee.function.Refresh;
            ...
            public void run() {
                ...
                switch (msg.getSign()) {
                    case 1:
                        ...
                        //创建refresh对象
                        Refresh refresh = new Refresh(SessionSet);
                        //把refresh.RefreshList()序列化再放到 responseMsgjson
                        responseMsgjson = objectMapper.writeValueAsString(refresh.RefreshList());
                        // 将返回消息发送回客户端
...
//                        System.out.println(Arrays.toString(msg.getSecretMsg()));
                        //创建一个Msg对象，处理收发消息
                        Msg copeMsg = new Msg(msg,username);
                        boolean flag = true;
                        //循环找 接受消息者在不在线
...
//                                Message SendMsg = new Message(31,username,null,secretMsgCopy);
                                ...
                                responseMsgjson = objectMapper.writeValueAsString(copeMsg.responseMsgYes());
                                System.out.println("发给" + basicRemote + " 了：" + responseMsgjson);
                                basicRemote.sendText(responseMsgjson);
                                flag = false;
                                break;
                            }
                        }
                        if(flag){
                            ...
                            responseMsgjson = objectMapper.writeValueAsString(copeMsg.responseMsgNot());
                            System.out.println("发给" + basicRemote + " 了：" + responseMsgjson);
                            basicRemote.sendText(responseMsgjson);
                        }
                        break;
    ...
    public void sendMessage(Message msg) {
        // 发送消息
          ...
          /*      if(msg.getSign() == 1){
                    ...
                    String username = (String) session.getUserProperties().get("username");
                    //创建一个Msg对象，处理收发消息
                    Msg copeMsg = new Msg(msg.getDate(),username);
                    //循环找 接受消息者在不在线
                    for (Session session : SessionSet) {
                        //当前遍历session 的name放到sendname
                        String sendname = (String) session.getUserProperties().get("username");
